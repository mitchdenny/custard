...
export function publicationTarget(version, kind, prNumber) {
    ...
    if (kind === 'release' && !match[1]) {
        return { registry: 'https://registry.npmjs.org', tag: 'latest' };
    }
    if ((kind === 'alpha' || kind === 'beta') && match[1]?.startsWith(`${kind}.`)) {
        return { registry: 'https://registry.npmjs.org', tag: kind };
    }
    if (kind === 'pr' && /^[1-9]\d*$/.test(prNumber ?? '') && match[1]?.startsWith(`pr.${prNumber}.`)) {
        return { registry: 'https://npm.pkg.github.com', tag: `pr-${prNumber}` };
    }
...
export function publishPackage({ version, kind, prNumber, tarball, manifest }, npm = runNpm) {
    const target = publicationTarget(version, kind, prNumber);
    ...
    const args = [
        'publish', tarball, '--ignore-scripts', '--registry', target.registry, '--tag', target.tag,
    ];
    if (kind !== 'pr') args.push('--access', 'public');
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
    ...
    publicationTarget(version, kind, prNumber);
    ...
    if (process.argv[2] === '--verify') {
        ...
        publishPackage({ version, kind, prNumber, tarball, manifest });
    }
}
