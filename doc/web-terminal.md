...
It currently uses an ASP.NET Core host and plain JavaScript ES modules. There
is no npm package, TypeScript build, WASM component, or embedded-JavaScript helper.
...
[`KgpAnimationState` checkpoint](../docs/muxer-protocol.md#kgpanimationstate-0x0c)
...
An atomic snapshot is internally consistent at a moment in time. It is not
necessarily a complete application frame; synchronized output is a separate
gap discussed below.
...
[provenance](../samples/WebTerminalDemo/wwwroot/fonts/cascadia-mono-nf/README.md).
...
[`WebTerminal.mount(container, options)`](../samples/WebTerminalDemo/wwwroot/web-terminal.js)
is a sample ES-module API, not a packaged SDK or injected-JavaScript helper.
...
Some cloud captures appeared to show partial scenes. The demos bracket frames
with synchronized-output sequences, but this path does not yet honor those
as presentation boundaries. That is a concrete gap and a plausible contributor,
not a proven diagnosis of every visual discontinuity.
...
|------|-------------|-----------|
| Completed frames | Atomic snapshots can catch an erase/delete followed by only part of a repaint. Core emitters and cloud demos use DEC mode 2026, but the receiver/presentation path does not enforce it. | Honor synchronized-output boundaries in the terminal/presentation contract, with bounded recovery for missing end markers. Test split writes and forced flush/reset cases; do not substitute an arbitrary delay. |
...
Packaging should follow clear component and hosting boundaries, not dictate
their design. A supported embedding API need not expose a stable wire protocol:
HWT1 can remain internal, with matching server and client artifacts distributed
and upgraded together. The sample `WebTerminal.mount` module already exists;
the distribution/helper names below are illustrative and do not exist as APIs.

| Option | Benefits | Questions / costs |
|--------|----------|-------------------|
| Embedded JavaScript resources, potentially exposed through a `Hex1bTerminalWebHelper`-style helper or endpoint | Convenient for .NET consumers; straightforward distribution alongside the compatible server implementation; can work without npm. | Worker/module/font asset URLs, MIME types, caching, CSP, trimming/publishing, and asset lifetime need a real contract. Returning a JavaScript string alone does not solve these. |
| An npm package, potentially `hex1b` or another available name | Natural frontend tooling integration, TypeScript declarations, framework wrappers, and independent asset hosting. | Package ownership/name availability, worker bundling, SSR-safe imports, backend compatibility, and coordinated releases need decisions. |
| Both distributions from one frontend source/build | Supports .NET-first and frontend-first consumers without separate renderers. | Requires deterministic shared artifacts, compatibility checks, and coordinated versioning rather than two maintained copies. |

A reasonable direction is TypeScript source with a small framework-neutral
component, and one build capable of producing npm and embedded assets. This is
a recommendation, not a decision to introduce a frontend toolchain immediately.
Keep the plain-JavaScript spike until extraction has a clear purpose.

The sample now mounts into a supplied element without global DOM ownership.
Production injection/embedding still needs explicit worker/asset configuration
and supported distribution. Avoid unrelated host styles. Support normal hosted
assets under a restrictive CSP;
do not assume every consumer permits inline scripts, `eval`, or `blob:` workers.
Keep server and client artifacts paired; do not infer cross-release wire
compatibility from the HWT1 name or version field.
...
|-----------|------------------------------|
| 1. Establish internal invariants and a corpus | Document the evolving HWT1 implementation, add viewport ownership and repeatable text/graphics/input recordings, and address synchronized-output and remaining geometry ambiguities. Equivalent cases should agree across browser, snapshots, SVG, and replay where their presentation capabilities overlap; this does not freeze a third-party wire contract. |
...
| 6. Package and publish | Select distribution(s), pin compatibility rules, and ship documentation and automated release coverage for worker/assets and sample integrations. Preserve one implementation across distributions. |
...
authorization boundaries. Final packaging and supported public API commitments
should follow those decisions. None requires a second browser terminal emulator.
...
| Projection and wire validation | [HWT1 internal implementation notes](web-terminal-protocol.md), [Hwt1RenderProjection](../src/Hex1b/Hwt1/Hwt1RenderProjection.cs), [protocol.js](../samples/WebTerminalDemo/wwwroot/protocol.js) |
| Worker and rendering | [terminal-worker.js](../samples/WebTerminalDemo/wwwroot/terminal-worker.js), [renderer.js](../samples/WebTerminalDemo/wwwroot/renderer.js) |
| Browser-independent peer/framebuffer regressions | [Node tests](../samples/WebTerminalDemo/tests/web-terminal.test.mjs): metadata authority validation and framebuffer/logical-geometry calculations using a stub GPU interface. |
...
| Mounting, playground, and browser/server input | [web-terminal.js](../samples/WebTerminalDemo/wwwroot/web-terminal.js), [main.js](../samples/WebTerminalDemo/wwwroot/main.js), [mouse-input.js](../samples/WebTerminalDemo/wwwroot/mouse-input.js), [Hwt1Input](../src/Hex1b/Hwt1/Hwt1Input.cs) |
