...
| `cloud-flicker.browser.js` | Real shell-launched Sixel/KGP cloud animations, sampling visible canvas pixels over at least 180 browser frames and ten received updates; KGP late/reconnected views must retain sprites and keep animating without re-uploading pixels. Build `samples/SixelCloudDemo` and `samples/KgpCloudDemo` in Release first. |
