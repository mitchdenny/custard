...
export class HistoryState {
  ...
  #pointRowId(point) {
    ...
    if (!point || !Number.isInteger(point.x) || point.x < 0 || point.x > 1023 ||
        ...
        typeof this.#history.rowIds[point.y] !== "string" || !this.#history.rowIds[point.y].length) {
      throw new Error("Terminal viewport is not ready for that selection point. Wait for the next frame and try again.");
    }
    return this.#history.rowIds[point.y];
  }
  ...
  begin(point, { mode, extend = false }) {
    ...
    this.#send({ type: "selection", action: extend ? "extend" : "start", mode, requestId,
      generation: this.#history.generation, rowId, column: point.x });
    this.#change();
  }
  ...
  copy() {
    ...
    this.#pendingCopy = { requestId, selectionRequestId: selection.requestId, generation: this.#history.generation,
      resolve, reject, timer };
    try {
      this.#send({ type: "copy", requestId, selectionRequestId: selection.requestId, generation: this.#history.generation });
    } catch (error) {
