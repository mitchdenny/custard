...
export class InputPolicy {
  ...
  #decision(value) {
    if (typeof value === "string" && routes.has(value)) return { route: value };
    ...
    if (hasRoute) {
      if (!routes.has(value.route)) throw new TypeError(`Unknown input route: ${value.route}`);
      return { route: value.route };
    }
    if (typeof value.action !== "function" && !this.#actions.has(value.action))
      throw new TypeError(`Unknown terminal action: ${value.action}`);
