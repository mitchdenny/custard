...
export class TerminalRenderer {
  static async create(canvas, scale, onFatal, font) {
    ...
    const renderer = new TerminalRenderer(canvas, scale, device, font);
  ...
  constructor(canvas, scale, device, font) {
    ...
    if (!this.context) throw new Error("Could not create an OffscreenCanvas WebGPU context");
    this.images = new Map();
    this.glyphs = new Map();
  ...
  async initialize() {
    ...
    this.raster = this.rasterCanvas.getContext("2d", { willReadFrequently: true });
    if (!this.raster) throw new Error("Worker glyph rasterization is unavailable");
  ...
  uploadGlyph({ key, cell, x, y, width, height }) {
    ...
    if (!this.fontMetrics.has(style)) {
      this.fontMetrics.set(style, measureFont(raster, this.font.cssFamily, style, scale, CELL_WIDTH, CELL_HEIGHT));
    }
    const metrics = this.fontMetrics.get(style);
