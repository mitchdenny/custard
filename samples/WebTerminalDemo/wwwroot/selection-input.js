...
export class SelectionGesture {
  ...
  begin(event, point, { tracking, historical, readOnly, selection }) {
    ...
    if (!local) return { owner: this.owner };
    const extend = !!event.shiftKey && ["valid", "pending"].includes(selection?.status) && selection.canExtend !== false;
    this.mode = extend ? selection.mode : event.altKey ? "rectangle"
  ...
  scrollPoint(point) {
    ...
    return { x: this.mode === "rectangle" ? this.endpoint.x : point.x, y: point.y };
  }
