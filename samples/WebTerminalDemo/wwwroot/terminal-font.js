...
export async function loadFont(configuration) {
  ...
  const fontSet = globalThis.fonts ?? globalThis.document?.fonts;
  ...
  for (const source of sources) {
    try {
      const face = new FontFace(family,
        source.local ? `local(${JSON.stringify(source.local)})` : `url(${JSON.stringify(source.url)})`,
        { weight: source.weight ?? "400", style: source.style ?? "normal" });
      loaded.push(await face.load());
    } catch (error) {
      throw new Error(`Could not load terminal font "${family}" from ${source.url ?? "local fonts"}`, { cause: error });
    }
  }
