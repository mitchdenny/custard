...
export class WebTerminal {
  ...
  #start(container) {
    ...
    this.#highlights = shadow.querySelector(".highlights");
    this.#inspection = shadow.querySelector(".inspection");
    ...
    this.#selectionUI = new SelectionUI({
      ...
      button: this.#inspection.querySelector(".copy-selection"), signal: this.#listeners.signal,
      ...
      runAction: (action, args) => this.runAction(action, args),
      onSelectionUI: this.#options.onSelectionUI,
      reportError: error => {
        this.#selectionUIError = error ? `Selection UI failed: ${error.message ?? String(error)}` : "";
    ...
    const inspect = operation => {
      ...
      catch (error) { this.#inspectionError = error.message; this.#inspectionChanged(); }
    };
    ...
    this.#bindKeyboard();
    this.#inspection.querySelector(".return-live").addEventListener("click", () => {
      this.runAction(TerminalAction.ScrollToLive).catch(error => this.#actionFailed(error));
    }, { signal: this.#listeners.signal });
    this.#inspection.querySelector(".copy-selection").addEventListener("click", () => {
      this.runAction(TerminalAction.CopySelection).catch(error => this.#actionFailed(error));
    }, { signal: this.#listeners.signal });
    shadow.querySelector(".viewport").addEventListener("pointerdown", event => {
      if (event.composedPath().includes(this.#selectionOverlay)) return;
      if (event.target.closest("button") || event.target === this.#canvas || event.target === this.#input) return;
    ...
    this.#worker = new Worker(new URL("./terminal-worker.js", import.meta.url), { type: "module", name: "Hex1b WebTerminal" });
    ...
    this.#worker.postMessage({ type: "init", canvas, url: url.href, scale, font }, [canvas]);
  }
  ...
  #fit() {
    ...
    this.#worker?.postMessage({ type: "viewport", width: Math.ceil(width * scale * dpr), height: Math.ceil(height * scale * dpr) });
  }
  ...
  #inspectionChanged() {
    ...
    if (this.#highlights) {
      ...
      const live = this.#inspection.querySelector(".return-live");
  ...
  #renderInspectionStatus() {
    ...
    const selection = this.selection;
    const viewport = this.viewport;
    const status = this.#inspection.querySelector(".inspection-message");
    status.textContent = this.#selectionUIError || this.#inspectionError ||
      (selection.status === "unavailable" ? "" : selection.message) ||
      (!viewport.following ? `${viewport.liveTop - viewport.top} rows above live` : "");
  ...
  #actionFailed(error) {
    ...
    this.#inspectionError = `Input action failed: ${error.message}`;
    this.#inspectionChanged();
    this.#options.onInputError?.(error);
  }
  ...
  async copySelection({ clear = false } = {}) {
    ...
    try {
      ...
      if (serial === this.#copySerial) {
        ...
        this.#inspectionError = `Copy failed: ${error.message}`;
      }
      throw error;
    } finally {
  ...
  #executeInputAction(decision, input) {
    this.runAction(decision.action, decision.args, input).catch(error => this.#actionFailed(error));
  }
  ...
  async runAction(action, args, input) {
    ...
    if (this.#actions.has(action)) return this.#actions.get(action)(this.inputContext, args, input);
    switch (action) {
      case TerminalAction.CopySelection: return this.copySelection(args);
      ...
      case TerminalAction.ScrollLines: return this.scrollLines(args);
  ...
  #bindKeyboard() {
    ...
    input.addEventListener("input", event => {
      ...
      clearTimeout(this.#compositionTimer);
      const text = compositionCommit ?? event.data ?? input.value;
      compositionCommit = null;
      if (text && event.inputType !== "insertFromPaste") this.#dispatchInput({ type: "text", text }, event);
      input.value = "";
    }, options);
  }
