...
/// </summary>
public readonly record struct TerminalCell(char Character, CustardColor? Foreground, CustardColor? Background)
{
...
/// </summary>
public sealed class CustardTerminal : ICustardTerminal, IDisposable
{
    ...
    private CustardColor? _currentForeground;
    private CustardColor? _currentBackground;
    ...
    public CustardTerminal(int width = 80, int height = 24)
    {
        Width = width;
        Height = height;
            ...
            switch (code)
            {
                ...
                case 38: // Extended foreground color
                    ...
                    else if (i + 4 < parts.Length && parts[i + 1] == "2")
                    {
                        ...
                        {
                            _currentForeground = CustardColor.FromRgb((byte)r, (byte)g, (byte)b);
                        }
                        i += 4;
                    }
                    break;
                case 48: // Extended background color
                    ...
                    else if (i + 4 < parts.Length && parts[i + 1] == "2")
                    {
                        if (int.TryParse(parts[i + 2], out var r) &&
                            ...
                            int.TryParse(parts[i + 4], out var b))
                        {
                            _currentBackground = CustardColor.FromRgb((byte)r, (byte)g, (byte)b);
                        }
                        i += 4;
                    }
                    break;
    ...
    private static CustardColor StandardColorFromCode(int code) => code switch
    {
        0 => CustardColor.FromRgb(0, 0, 0),       // Black
        1 => CustardColor.FromRgb(128, 0, 0),     // Red
        2 => CustardColor.FromRgb(0, 128, 0),     // Green
        3 => CustardColor.FromRgb(128, 128, 0),   // Yellow
        4 => CustardColor.FromRgb(0, 0, 128),     // Blue
        5 => CustardColor.FromRgb(128, 0, 128),   // Magenta
        6 => CustardColor.FromRgb(0, 128, 128),   // Cyan
        7 => CustardColor.FromRgb(192, 192, 192), // White
        _ => CustardColor.FromRgb(128, 128, 128)
    };

    private static CustardColor BrightColorFromCode(int code) => code switch
    {
        0 => CustardColor.FromRgb(128, 128, 128), // Bright Black (Gray)
        1 => CustardColor.FromRgb(255, 0, 0),     // Bright Red
        2 => CustardColor.FromRgb(0, 255, 0),     // Bright Green
        3 => CustardColor.FromRgb(255, 255, 0),   // Bright Yellow
        4 => CustardColor.FromRgb(0, 0, 255),     // Bright Blue
        5 => CustardColor.FromRgb(255, 0, 255),   // Bright Magenta
        6 => CustardColor.FromRgb(0, 255, 255),   // Bright Cyan
        7 => CustardColor.FromRgb(255, 255, 255), // Bright White
        _ => CustardColor.FromRgb(192, 192, 192)
    };

    private static CustardColor Color256FromIndex(int index)
    {
        if (index < 16)
        {
            // Standard colors
        ...
        else if (index < 232)
        {
            ...
            var g = ((index / 6) % 6) * 51;
            var b = (index % 6) * 51;
            return CustardColor.FromRgb((byte)r, (byte)g, (byte)b);
        }
        else
        {
            // Grayscale
            var gray = (index - 232) * 10 + 8;
            return CustardColor.FromRgb((byte)gray, (byte)gray, (byte)gray);
        }
    }
}
